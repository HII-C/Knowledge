...
class SumRelation:
    ...
    def get_occurences(self, output_table, n=10000, semmed_db='semmed', output_db='derived'):
        ...
        exec_str = f'''
                    CREATE TABLE {output_db}.{output_table}(SUBJECT_CUI VARCHAR(9), OBJECT_CUI VARCHAR(9), RELATION SMALLINT, COUNT INT)
                    AS SELECT pred.SUBJECT_CUI, pred.OBJECT_CUI, pred.PREDICATE as RELATION, COUNT(*) AS COUNT
                    FROM {semmed_db}.PREDICATION AS pred
                    GROUP BY pred.SUBJECT_CUI, pred.OBJECT_CUI, pred.PREDICATE limit {n}'''
