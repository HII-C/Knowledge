...
class PatientPopulation:
    ...
    def random_pid_generation(self, table, n=1000) -> List[int]:
        ...
        id_tup = tuple(randint(0, self.population_size, n))
    ...
    def form_lhs_bin(self, concept: ConceptType) -> pd.DataFrame:
        ...
        self.lhs = pd.DataFrame(np.zeros(shape=(self.population_size,
    ...
    def form_lhs_cont(self, concept: ConceptType) -> pd.DataFrame:
    ...
    def form_rhs(self, target_type: ConceptType,
                 target_concept: str) -> pd.Series:
        ''''''
        self.rhs = pd.Series(np.zeros(self.population_size),
                             index=self.subject_id)
    ...
    def form_rhs_mult_and(self, target_type: ConceptType,
                          target_concept: List[str]) -> pd.Series:
        ''''''
        self.rhs = pd.Series(np.zeros(self.population_size),
                             index=self.subject_id)
        rows = self.database.rhs_bin(self.subject_id,
