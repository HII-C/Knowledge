...
class CreateUMLSCUIMapping:
    ...
    def create_labevents_UMLS_all(self,
                                  output_table: str = 'labevents_UMLS',
                                  output_db: str = 'derived',
                                  mimic_db: str = 'mimic',
                                  input_table: str = 'LABEVENTS',
                                  mappings_table: str = 'ItemIdToCUI',
    ...
    def create_labevents_UMLS_w_VALUENUM(self,
                                  output_table: str = 'labevents_UMLS',
                                  output_db: str = 'derived',
                                  mimic_db: str = 'mimic',
                                  input_table: str = 'LABEVENTS',
    ...
    def diagnoses_ICD_to_CUI(self, output_table, n=10000, output_db='derived'):
        ...
        exec_str = f'''CREATE TABLE {output_db}.{temp_table}
                            (CUI CHAR(8),
                             AUI VARCHAR(9),
                             SAB VARCHAR(40),
                             ICD9_CODE VARCHAR(255)) 
                        AS SELECT
                            umls.MRCONSO.CUI as CUI,
                            umls.MRCONSO.AUI as AUI,
                            umls.MRCONSO.SAB as SAB,
                            mimic.D_ICD_DIAGNOSES.ICD9_CODE as ICD9_CODE
                        FROM umls.MRCONSO
                            INNER JOIN mimic.D_ICD_DIAGNOSES 
                                ON 
                            umls.MRCONSO.STR = mimic.D_ICD_DIAGNOSES.SHORT_TITLE 
                        limit {n}'''
        ...
        exec_str = f'''CREATE TABLE {output_db}.{output_table}
                            (CUI CHAR(8),
                             ...
                             SEQ_NUM INT)
                        AS SELECT 
                            derived.ICDTOCUI.CUI as CUI, 
                            mimic.DIAGNOSES_ICD.ROW_ID as ROW_ID, 
                            mimic.DIAGNOSES_ICD.SUBJECT_ID as SUBJECT_ID, 
                            mimic.DIAGNOSES_ICD.HADM_ID as HADM_ID, 
                            mimic.DIAGNOSES_ICD.SEQ_NUM as SEQ_NUM 
                        FROM mimic.DIAGNOSES_ICD 
                            INNER JOIN derived.ICDTOCUI 
                                ON 
                            mimic.DIAGNOSES_ICD.ICD9_CODE = derived.ICDTOCUI.ICD9_CODE
                        limit {n}'''
    ...
    def create_diagnoses_UMLS(self, output_table, n=10000, output_db='derived'):
        ...
        exec_str = f'''CREATE TABLE {output_db}.{output_table}
                            (CUI CHAR(8),
                             ...
                             FLAG VARCHAR(255))
                        AS SELECT 
                            mimic.ItemIdToCUI.CUI as CUI,
                            mimic.LABEVENTS.ROW_ID as ROW_ID, 
                            mimic.LABEVENTS.SUBJECT_ID as SUBJECT_ID,
                            mimic.LABEVENTS.HADM_ID as HADM_ID,
                            mimic.LABEVENTS.CHARTTIME as CHARTTIME, 
                            mimic.LABEVENTS.VALUE as VALUE,
                            mimic.LABEVENTS.VALUENUM as VALUENUM,
                            mimic.LABEVENTS.VALUEUOM as VALUEUOM,
                            mimic.LABEVENTS.FLAG as FLAG
                        FROM 
                            mimic.LABEVENTS INNER JOIN
                                ON
                            mimic.ItemIdToCUI ON mimic.LABEVENTS.ITEMID = mimic.ItemIdToCUI.ITEMID 
                        limit {n}'''
